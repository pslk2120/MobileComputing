...
class NAmericaScreen extends StatefulWidget {
  @override
  _NAmericaScreenState createState() => _NAmericaScreenState();
}

class _NAmericaScreenState extends State<NAmericaScreen> {
  List<bool> _isCheckedCountryList = List.filled(Lists().northAmerica.length, false); // For overall countries
  Map<int, Map<String, List<bool>>> _isCheckedDetailMap = {}; // For details of each country
  List<bool> _isExpandedList = List.filled(Lists().northAmerica.length, false); // For expansion state of each country
  List<bool> _isCountryFullyCheckedList = List.filled(Lists().northAmerica.length, false);

  @override
  Widget build(BuildContext context) {
    int checkedCountries = _isCheckedCountryList.where((isChecked) => isChecked).length;
    int totalCountries = Lists().northAmerica.length;
    ...
    return Scaffold(
        ...
        children: [
          ...
          Expanded(
            child: ListView.builder(
              itemCount: Lists().northAmerica.length,
                    ...
                    Container(
                      ...
                      child: ListTile(
                        ...
                        leading: ImageCheckbox(
                          ...
                          imagePath: Lists().northAmerica[index].flag,
                        ),
                        trailing: Icon(
                    ...
                    if (_isExpandedList[index])
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                                if (Lists().northAmerica[index].capitol.isEmpty)
                                  _buildChecklistEmpty('Capitol')
                                  else
                                    _buildChecklist('Capitol',Lists().northAmerica[index].capitol,index,'Capitol'),
                              
                                if (Lists().northAmerica[index].states.isEmpty)
                                  _buildChecklistEmpty('States/Provinces')
                                  else
                                    _buildChecklist('States/Provinces',Lists().northAmerica[index].states.join(', '),index,'States/Provinces'),

                                if (Lists().northAmerica[index].capitals.isEmpty)
                                    _buildChecklistEmpty('State Capitals')
                                  else
                                    _buildChecklist('State Capitals',Lists().northAmerica[index].capitals.join(', '),index,'State Capitals'),

                                if (Lists().northAmerica[index].cities.isEmpty)
                                  _buildChecklistEmpty('Cities')
                                  else
                                    _buildChecklist('Cities',Lists().northAmerica[index].cities.join(', '),index,'Cities'),
                                
                                if (Lists().northAmerica[index].mustSee.isEmpty)
                                  _buildChecklistEmpty('Must-See')
                                  else
                                    _buildChecklist('Must-See',Lists().northAmerica[index].mustSee.join(', '),index,'Must-See'),
                                                                                            
                                if (Lists().northAmerica[index].nationalParks.isEmpty)
                                    _buildChecklistEmpty('National Parks')
                                    else
                                      _buildChecklist('National Parks',Lists().northAmerica[index].nationalParks.join(', '),index,'National Parks'),
                                  
                                if (Lists().northAmerica[index].islands.isEmpty)
                                    _buildChecklistEmpty('Islands/Peninsulas')
                                    else
                                      _buildChecklist('Islands/Peninsulas',Lists().northAmerica[index].islands.join(', '),index,'Islands/Peninsulas'),
                                  
                                if (Lists().northAmerica[index].foodAndDrinks.isEmpty)
                                    _buildChecklistEmpty('Food and Drinks')
                                    else
                                      _buildChecklist('Food and Drinks',Lists().northAmerica[index].foodAndDrinks.join(', '),index,'Food and Drinks'),
                              ],
                            ),
                      ),
                  ],
      ...
      bottomNavigationBar: CustomBottomNavigationBar(1),
                                    ...
                                    children: itemList.sublist(0, halfItemCount).map((item) {
                                      return ListTile(
                                        ...
                                        leading: ImageCheckbox(
                                          ...
                                          imagePath: Lists().northAmerica[countryIndex].flag, // Use the flag path from the list
                                        ),
                                      );
                                    }).toList(),
                                  ),
                ...
                void _updateChecklists(int index) {
                  setState(() {
                    if (_isExpandedList[index]) {
                      final countryDetails = Lists().northAmerica[index];
                ...
                bool _areAllChecklistsChecked(int countryIndex) {
                  if (_isExpandedList[countryIndex]) {
                    final countryDetails = Lists().northAmerica[countryIndex];
                    final nonEmptyDetails = {
