...
class OceaniaScreen extends StatefulWidget {
  @override
  _OceaniaScreenState createState() => _OceaniaScreenState();
}

class _OceaniaScreenState extends State<OceaniaScreen> {
  List<bool> _isCheckedCountryList = List.filled(Lists().oceania.length, false); // For overall countries
  Map<int, Map<String, List<bool>>> _isCheckedDetailMap = {}; // For details of each country
  List<bool> _isExpandedList = List.filled(Lists().oceania.length, false); // For expansion state of each country
  List<bool> _isCountryFullyCheckedList = List.filled(Lists().oceania.length, false);

  @override
  Widget build(BuildContext context) {
    int checkedCountries = _isCheckedCountryList.where((isChecked) => isChecked).length;
    int totalCountries = Lists().oceania.length;
    ...
    return Scaffold(
        ...
        children: [
          ...
          Expanded(
            child: ListView.builder(
              itemCount: Lists().oceania.length,
                    ...
                    Container(
                      ...
                      child: ListTile(
                        ...
                        leading: ImageCheckbox(
                          ...
                          imagePath: Lists().oceania[index].flag,
                        ),
                        trailing: Icon(
                    ...
                    if (_isExpandedList[index])
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                              if (Lists().oceania[index].capitol.isEmpty)
                                _buildChecklistEmpty('Capitol')
                                else
                                  _buildChecklist('Capitol',Lists().oceania[index].capitol,index,'Capitol'),
                            
                              if (Lists().oceania[index].states.isEmpty)
                                _buildChecklistEmpty('States/Provinces')
                                else
                                  _buildChecklist('States/Provinces',Lists().oceania[index].states.join(', '),index,'States/Provinces'),

                              if (Lists().oceania[index].capitals.isEmpty)
                                  _buildChecklistEmpty('State Capitals')
                                else
                                  _buildChecklist('State Capitals',Lists().oceania[index].capitals.join(', '),index,'State Capitals'),

                              if (Lists().oceania[index].cities.isEmpty)
                                _buildChecklistEmpty('Cities')
                                else
                                  _buildChecklist('Cities',Lists().oceania[index].cities.join(', '),index,'Cities'),
                              
                              if (Lists().oceania[index].mustSee.isEmpty)
                                _buildChecklistEmpty('Must-See')
                                else
                                  _buildChecklist('Must-See',Lists().oceania[index].mustSee.join(', '),index,'Must-See'),
                                                                                          
                              if (Lists().oceania[index].nationalParks.isEmpty)
                                  _buildChecklistEmpty('National Parks')
                                  else
                                    _buildChecklist('National Parks',Lists().oceania[index].nationalParks.join(', '),index,'National Parks'),
                                
                              if (Lists().oceania[index].islands.isEmpty)
                                  _buildChecklistEmpty('Islands/Peninsulas')
                                  else
                                    _buildChecklist('Islands/Peninsulas',Lists().oceania[index].islands.join(', '),index,'Islands/Peninsulas'),
                                
                              if (Lists().oceania[index].foodAndDrinks.isEmpty)
                                  _buildChecklistEmpty('Food and Drinks')
                                  else
                                    _buildChecklist('Food and Drinks',Lists().oceania[index].foodAndDrinks.join(', '),index,'Food and Drinks'),
                            ],
                        ),
                      ),
                  ],
      ...
      bottomNavigationBar: CustomBottomNavigationBar(1),
                                    ...
                                    children: itemList.sublist(0, halfItemCount).map((item) {
                                      return ListTile(
                                        ...
                                        leading: ImageCheckbox(
                                          ...
                                          imagePath: Lists().oceania[countryIndex].flag, // Use the flag path from the list
                                        ),
                                      );
                                    }).toList(),
                                  ),
                ...
                void _updateChecklists(int index) {
                  setState(() {
                    if (_isExpandedList[index]) {
                      final countryDetails = Lists().oceania[index];
                ...
                bool _areAllChecklistsChecked(int countryIndex) {
                  if (_isExpandedList[countryIndex]) {
                    final countryDetails = Lists().oceania[countryIndex];
                    final nonEmptyDetails = {
