...
class AfricaScreen extends StatefulWidget {
  @override
  _AfricaScreenState createState() => _AfricaScreenState();
}

class _AfricaScreenState extends State<AfricaScreen> {
  List<bool> _isCheckedCountryList = List.filled(Lists().africa.length, false); // For overall countries
  Map<int, Map<String, List<bool>>> _isCheckedDetailMap = {}; // For details of each country
  List<bool> _isExpandedList = List.filled(Lists().africa.length, false); // For expansion state of each country
  List<bool> _isCountryFullyCheckedList = List.filled(Lists().africa.length, false);

  @override
  Widget build(BuildContext context) {
    int checkedCountries = _isCheckedCountryList.where((isChecked) => isChecked).length;
    int totalCountries = Lists().africa.length;
                    ...
                    Container(
                      ...
                      child: ListTile(
                        ...
                        leading: ImageCheckbox(
                          ...
                          imagePath: Lists().africa[index].flag,
                        ),
                        trailing: Icon(
                    ...
                    if (_isExpandedList[index])
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                                if (Lists().africa[index].capitol.isEmpty)
                                  _buildChecklistEmpty('Capitol')
                                  else
                                    _buildChecklist('Capitol',Lists().africa[index].capitol,index,'Capitol'),
                              
                                if (Lists().africa[index].states.isEmpty)
                                  _buildChecklistEmpty('States/Provinces')
                                  else
                                    _buildChecklist('States/Provinces',Lists().africa[index].states.join(', '),index,'States/Provinces'),

                                if (Lists().africa[index].capitals.isEmpty)
                                    _buildChecklistEmpty('State Capitals')
                                  else
                                    _buildChecklist('State Capitals',Lists().africa[index].capitals.join(', '),index,'State Capitals'),

                                if (Lists().africa[index].cities.isEmpty)
                                  _buildChecklistEmpty('Cities')
                                  else
                                    _buildChecklist('Cities',Lists().africa[index].cities.join(', '),index,'Cities'),
                                
                                if (Lists().africa[index].mustSee.isEmpty)
                                  _buildChecklistEmpty('Must-See')
                                  else
                                    _buildChecklist('Must-See',Lists().africa[index].mustSee.join(', '),index,'Must-See'),
                                                                                            
                                if (Lists().africa[index].nationalParks.isEmpty)
                                    _buildChecklistEmpty('National Parks')
                                    else
                                      _buildChecklist('National Parks',Lists().africa[index].nationalParks.join(', '),index,'National Parks'),
                                  
                                if (Lists().africa[index].islands.isEmpty)
                                    _buildChecklistEmpty('Islands/Peninsulas')
                                    else
                                      _buildChecklist('Islands/Peninsulas',Lists().africa[index].islands.join(', '),index,'Islands/Peninsulas'),
                                  
                                if (Lists().africa[index].foodAndDrinks.isEmpty)
                                    _buildChecklistEmpty('Food and Drinks')
                                    else
                                      _buildChecklist('Food and Drinks',Lists().africa[index].foodAndDrinks.join(', '),index,'Food and Drinks'),
                              ],
                            ),
                      ),
                  ],
                                    ...
                                    children: itemList.sublist(0, halfItemCount).map((item) {
                                      return ListTile(
                                        ...
                                        leading: ImageCheckbox(
                                          ...
                                          imagePath: Lists().africa[countryIndex].flag, // Use the flag path from the list
                                        ),
                                      );
                                    }).toList(),
                                  ),
  ...
  void _updateChecklists(int index) {
                  setState(() {
                    if (_isExpandedList[index]) {
                      final countryDetails = Lists().africa[index];
  ...
  bool _areAllChecklistsChecked(int countryIndex) {
                  if (_isExpandedList[countryIndex]) {
                    final countryDetails = Lists().africa[countryIndex];
                    final nonEmptyDetails = {
